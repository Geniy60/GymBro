function exerciseCountLabel(count: number) {
  if (count % 10 === 1 && count % 100 !== 11) {
    return `${count} упражнение`;
  }

  if ([2, 3, 4].includes(count % 10) && ![12, 13, 14].includes(count % 100)) {
    return `${count} упражнения`;
  }

  return `${count} упражнений`;
}

function historyRecordCountLabel(count: number) {
  if (count % 10 === 1 && count % 100 !== 11) {
    return `${count} запись`;
  }

  if ([2, 3, 4].includes(count % 10) && ![12, 13, 14].includes(count % 100)) {
    return `${count} записи`;
  }

  return `${count} записей`;
}

function setCountLabel(count: number) {
  if (count % 10 === 1 && count % 100 !== 11) {
    return `${count} подход`;
  }

  if ([2, 3, 4].includes(count % 10) && ![12, 13, 14].includes(count % 100)) {
    return `${count} подхода`;
  }

  return `${count} подходов`;
}

export const strings = {
  app: {
    title: 'GymBro',
  },
  tabs: {
    machines: 'Упражнения',
    stats: 'Статистика',
    workouts: 'Тренировки',
  },
  search: {
    machines: 'Поиск упражнения',
    workouts: 'Поиск тренировки',
  },
  empty: {
    machines: {
      title: 'Упражнений пока нет',
      message: 'Здесь появятся упражнения, которые ты добавишь для своих занятий.',
    },
    workouts: {
      title: 'Тренировок пока нет',
      message: 'Нажми кнопку внизу, чтобы начать первую тренировку в зале.',
    },
    filtered: {
      title: 'Ничего не найдено',
      message: 'Попробуй изменить запрос или сбросить поиск.',
    },
  },
  actions: {
    cancel: 'Отмена',
    addIcon: '+',
    delete: 'Удалить',
    dontSave: 'Не сохранять',
    editIcon: '✎',
    finish: 'Завершить',
    ok: 'ОК',
    openSettings: 'Открыть настройки',
    restore: 'Восстановить',
    retry: 'Повторить',
    resetSearch: 'Сбросить поиск',
    save: 'Сохранить',
  },
  settings: {
    bodyMeasurements: 'Замеры тела',
    bodyMeasurementsDescription: 'Вес, талия, бедра и график изменений',
    changeUser: 'Сменить',
    currentUser: 'Пользователь',
    noUser: 'Не выбран',
    restTimer: 'Таймер отдыха',
    restTimerDescription: 'Длительность таймера между подходами',
    theme: 'Тема',
    themeDescription: 'Светлое или тёмное оформление приложения',
    themeTitle: 'Тема',
    lightTheme: 'Светлая',
    darkTheme: 'Тёмная',
    title: 'Настройки',
  },
  restTimer: {
    activeLabel: (time: string) => `Отдых ${time}`,
    invalidSeconds: 'Введите целое число секунд больше нуля.',
    notificationBody: 'Пора продолжать тренировку.',
    notificationChannelName: 'Таймер отдыха',
    notificationTitle: 'Отдых закончился',
    ongoingNotificationBody: 'Телефон сообщит, когда отдых закончится.',
    ongoingNotificationTitle: 'Таймер отдыха идет',
    secondsLabel: 'Секунды',
    secondsPlaceholder: '90',
    settingTitle: 'Таймер отдыха',
    startLabel: 'Отдых',
  },
  bodyMeasurements: {
    addTitle: 'Новые показания',
    abdomenCm: 'Живот, см',
    cancelEdit: 'Отменить редактирование',
    chartEmpty: 'Добавь хотя бы два значения, чтобы увидеть график.',
    chestCm: 'Грудь, см',
    deleteMessage: 'Эта запись будет удалена из истории замеров.',
    deleteTitle: 'Удалить замер?',
    editTitle: 'Редактировать показания',
    empty: 'Замеров пока нет.',
    hipsCm: 'Бедра, см',
    invalidValue: 'Проверь значения: если поле заполнено, там должно быть число 0 или больше.',
    listTitle: 'История',
    loading: 'Загружаем замеры...',
    metricHips: 'Бедра',
    metricAbdomen: 'Живот',
    metricChest: 'Грудь',
    metricWaist: 'Талия',
    metricWeight: 'Вес',
    saveEmpty: 'Заполни хотя бы одно поле.',
    saved: 'Показания сохранены.',
    title: 'Замеры тела',
    waistCm: 'Талия, см',
    weightKg: 'Вес, кг',
  },
  machineImages: {
    noImage: 'Нет изображения',
  },
  machineTracking: {
    cardio: 'Кардио',
    strength: 'Силовое',
  },
  stats: {
    totalWorkouts: 'Всего',
    monthWorkouts: 'За месяц',
    chartTitle: 'Последние 6 месяцев',
    cardioTitle: 'Кардио',
    cardioEmpty: 'Кардио пока не записывали.',
    cardioValue: (distanceKm: string, elevationMeters: string, durationMinutes: string) =>
      `${distanceKm} км · ${elevationMeters} м · ${durationMinutes} мин`,
    historyListTitle: 'Посмотреть историю упражнения',
    historyListEmpty: 'Истории упражнений пока нет.',
    historySearchPlaceholder: 'Поиск упражнения',
    historyRecordCountTitle: 'Записей',
    historyRecordCount: historyRecordCountLabel,
    historySetsEmpty: 'Подходов в этой записи нет.',
    historySetsLoading: 'Загружаем подходы...',
    maxesTitle: 'Максимумы по упражнениям',
    emptyMaxes: 'Пока нет подходов с весом.',
    historyEmpty: 'По этому упражнению пока нет истории.',
    historyTitle: 'История упражнения',
    maxWeight: (weightKg: string, date: string) => `${weightKg} кг · ${date}`,
    noWeight: 'Без веса',
    openHistoryItem: (name: string) => `Открыть историю упражнения ${name}`,
    setCount: setCountLabel,
    strengthHistoryMaxTitle: 'Максимум',
    workoutMax: (weightKg: string) => `Макс. ${weightKg} кг`,
  },
  users: {
    selectMessage: 'Выбери, чьи тренировки показывать на этом телефоне.',
    selectTitle: 'Кто тренируется?',
    selectUser: (name: string) => `Выбрать пользователя ${name}`,
  },
  alerts: {
    deleteMachineTitle: 'Удалить упражнение?',
    deleteMachineMessage: (name: string) =>
      `Упражнение «${name}» будет удалено из списка.`,
    deleteWorkoutTitle: 'Удалить тренировку?',
    deleteWorkoutMessage: (name: string) =>
      `Тренировка «${name}» будет удалена из списка.`,
    deleteExerciseTitle: 'Удалить упражнение?',
    deleteExerciseMessage: 'Все подходы этого упражнения будут удалены.',
    exitWorkoutTitle: 'Сохранить тренировку?',
    exitWorkoutMessage: 'Перед выходом можно сохранить введенные подходы.',
    invalidWorkoutTitle: 'Проверь подходы',
    restoreWorkoutDraftTitle: 'Восстановить тренировку?',
    restoreWorkoutDraftMessage:
      'Найден незавершенный черновик. Можно продолжить с последними введенными подходами.',
    notificationPermissionTitle: 'Уведомления выключены',
    notificationPermissionMessage:
      'Таймер запущен, но телефон не покажет уведомление без разрешения.',
    exactAlarmPermissionTitle: 'Точный таймер выключен',
    exactAlarmPermissionMessage:
      'Android может откладывать таймер при выключенном экране. Разреши приложению будильники и напоминания в системных настройках, затем запусти таймер еще раз.',
    storageLoadTitle: 'Не удалось загрузить данные',
    storageLoadMessage: 'Проверь подключение к интернету и попробуй еще раз.',
    storageSaveTitle: 'Не удалось сохранить данные',
    storageSaveMessage: 'Проверь подключение к интернету и попробуй снова.',
    userSaveTitle: 'Не удалось запомнить пользователя',
    userSaveMessage:
      'Выбор применен сейчас, но может не сохраниться после перезапуска приложения.',
  },
  forms: {
    machine: {
      addTitle: 'Новое упражнение',
      editTitle: 'Редактировать упражнение',
      nameLabel: 'Название',
      namePlaceholder: 'Например, жим ногами',
      muscleGroupLabel: 'Группы мышц',
      muscleGroupHint: 'Выбери одну или несколько групп.',
      noteLabel: 'Заметка',
      notePlaceholder: 'Посадка, настройки, важные ощущения',
      trackingTypeLabel: 'Тип упражнения',
      errors: {
        nameRequired: 'Введите название упражнения.',
      },
    },
    workout: {
      nameLabel: 'Название',
      namePlaceholder: 'Введите название тренировки',
    },
  },
  workouts: {
    addExerciseTitle: 'Добавить упражнение',
    addMachineToWorkout: (name: string) => `Добавить ${name} в тренировку`,
    addSet: 'Добавить подход',
    clearExerciseSets: 'Очистить подходы',
    collapseExercise: 'Свернуть упражнение',
    defaultName: 'Тренировка',
    defaultNameWithDate: (weekday: string, date: string) => `${weekday} ${date}`,
    deleteExercise: 'Удалить упражнение',
    deleteSet: 'Удалить подход',
    emptyExercises: 'Добавь упражнение выше, затем вноси подходы по одному.',
    expandExercise: 'Развернуть упражнение',
    loadingMore: 'Загружаем еще...',
    machineSearchPlaceholder: 'Поиск упражнения',
    noMachinesTitle: 'Упражнений пока нет',
    noMachinesMessage: 'Сначала добавь упражнения во вкладке «Упражнения».',
    addSuggestedMachines: 'К тренировке!',
    openSuggestMachines: 'Подобрать упражнения',
    resuggestMachines: 'Подобрать заново',
    repsLabel: 'Повторения',
    repsPlaceholder: 'Повт.',
    repsUnit: 'р.',
    repsValue: (reps: string) => `${reps} р.`,
    recordBadge: 'Рекорд',
    sessionTitle: 'Тренировка',
    saved: 'Тренировка сохранена.',
    saveFailed: 'Не удалось сохранить. Проверь интернет и повтори.',
    saving: 'Сохраняем тренировку...',
    invalidRepsMessage:
      'Проверь повторы: если поле заполнено, там должно быть целое число больше нуля.',
    invalidWeightMessage:
      'Проверь вес: если поле заполнено, там должно быть число 0 или больше.',
    invalidCardioMessage:
      'Проверь кардио: если поле заполнено, там должно быть число 0 или больше.',
    cardioDurationLabel: 'Время, мин',
    cardioDurationPlaceholder: 'мин',
    cardioDistanceLabel: 'Дистанция, км',
    cardioDistancePlaceholder: 'км',
    cardioInclineLabel: 'Наклон, %',
    cardioInclinePlaceholder: '%',
    cardioElevationLabel: 'Высота, м',
    cardioElevationPlaceholder: 'м',
    cardioSpeedLabel: 'Скорость, км/ч',
    cardioSpeedPlaceholder: 'км/ч',
    clearCardioSet: 'Очистить кардио',
    setNoteLabel: 'Заметка к подходу',
    setNotePlaceholder: 'Заметка к подходу',
    setNumber: (number: number) => `${number}.`,
    suggestCountTitle: 'Сколько упражнений?',
    suggestMachinesButton: 'Подобрать',
    suggestMachinesTitle: 'Подбор упражнений',
    suggestMuscleGroupsTitle: 'Что тренируем?',
    suggestNoMatches: 'Подходящих упражнений не нашлось. Попробуй выбрать другие группы.',
    suggestPickMusclesHint: 'Выбери одну или несколько групп мышц.',
    suggestPreviewEmpty: 'Нажми «Подобрать», чтобы собрать варианты.',
    suggestPreviewTitle: 'Подойдет сейчас',
    suggestedMachine: (name: string) => `Подобранное упражнение ${name}`,
    selectSuggestMachineCount: (count: number) => `Выбрать ${exerciseCountLabel(count)}`,
    startWorkout: 'Начать тренировку',
    toggleSetNote: 'Показать заметку к подходу',
    toggleSuggestMuscleGroup: (name: string) => `Выбрать группу мышц ${name}`,
    unknownMonth: 'Без даты',
    weightLabel: 'Вес, кг',
    weightPlaceholder: 'кг',
    weightUnit: 'кг',
    weightValue: (weightKg: string) => `${weightKg} кг`,
  },
  accessibility: {
    addMachine: 'Добавить упражнение',
    addWorkout: 'Добавить тренировку',
    back: 'Назад',
    bodyMeasurements: 'Открыть замеры тела',
    bodyMeasurementMetric: (name: string) => `Показать график: ${name}`,
    cancelRestTimer: 'Остановить таймер отдыха',
    deleteBodyMeasurement: 'Удалить замер',
    deleteMachine: 'Удалить упражнение',
    deleteWorkout: 'Удалить тренировку',
    editMachine: 'Редактировать упражнение',
    editBodyMeasurement: 'Редактировать замер',
    editWorkout: 'Редактировать тренировку',
    finishWorkout: 'Завершить тренировку',
    openMachinePicker: 'Открыть выбор упражнения',
    repeatWorkout: 'Повторить тренировку',
    refreshData: 'Обновить данные',
    restTimerSettings: 'Открыть настройки таймера отдыха',
    settings: 'Открыть настройки',
    toggleHistorySets: (date: string) => `Показать подходы за ${date}`,
    themeSettings: 'Открыть настройки темы',
    search: 'Поиск',
    clearSearch: 'Очистить поиск',
    changeUser: 'Сменить пользователя',
    saveMachine: 'Сохранить упражнение',
    saveRestTimerSettings: 'Сохранить настройки таймера отдыха',
    saveWorkout: 'Сохранить тренировку',
    startRestTimer: 'Запустить таймер отдыха',
    startWorkout: 'Начать тренировку',
  },
  muscleGroups: {
    labels: {
      chest: 'Грудь',
      back: 'Спина',
      shoulders: 'Плечи',
      biceps: 'Бицепс',
      triceps: 'Трицепс',
      forearms: 'Предплечья',
      abs: 'Пресс',
      glutes: 'Ягодицы',
      quads: 'Квадрицепс',
      hamstrings: 'Бицепс бедра',
      calves: 'Икры',
      traps: 'Трапеции',
      adductors: 'Приводящие',
      abductors: 'Отводящие',
      lowerBack: 'Поясница',
    },
    legacyAliases: {
      chest: ['грудь', 'грудные'],
      back: ['спина', 'широчайшие'],
      shoulders: ['плечи', 'дельты'],
      biceps: ['бицепс', 'бицепсы'],
      triceps: ['трицепс', 'трицепсы'],
      forearms: ['предплечья', 'предплечье'],
      abs: ['пресс', 'кор'],
      glutes: ['ягодицы', 'ягодичные'],
      quads: ['квадрицепс', 'квадрицепсы', 'ноги', 'бедра'],
      hamstrings: ['бицепс бедра', 'задняя поверхность бедра'],
      calves: ['икры', 'голени'],
      traps: ['трапеции', 'трапеция'],
      adductors: ['приводящие'],
      abductors: ['отводящие'],
      lowerBack: ['поясница', 'разгибатели спины'],
    },
  },
} as const;
